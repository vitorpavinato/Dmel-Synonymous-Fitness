...
def readfile(infilename):
    ...
    with open(infilename, "r") as f:
        ...
        for li,line in enumerate(f):
            if li == 0:
                # Store header line to determine column positions
            ...
            if len(line) > 2:
                ...
                if header:
                    # Default to original column positions (0, 1, 2, 3)
                    neut_total_col = 0
                    neut_derived_col = 1
                    sel_total_col = 2
                    sel_derived_col = 3
                    
                    # If we have the new format with Neut_Chromosome etc., find the correct columns
                    if "Neut_total_allele_count" in header:
                        neut_total_col = header.index("Neut_total_allele_count")
                    if "Neut_derived_count" in header:
                        neut_derived_col = header.index("Neut_derived_count")
                    if "Selected_total_allele_count" in header:
                        sel_total_col = header.index("Selected_total_allele_count")
                    if "Selected_derived_count" in header:
                        sel_derived_col = header.index("Selected_derived_count")
                    
                # Process neutral counts
                snc = int(l[neut_total_col])
                sa = int(l[neut_derived_col])
                if Nminnc > snc:
                    Nminnc = snc
                ...
                # Process selected counts
                snc = int(l[sel_total_col])
                sa = int(l[sel_derived_col])
                if Sminnc > snc:
                    Sminnc = snc

...
main() {
    ...
    # Set up paths
    local inputdir="${WORKDIR}/${popcode}_imputed_vcf_n${DOWNSAMPLE}/codon_specific_results"
    local outputdir="${WORKDIR}/${popcode}_imputed_vcf_n${DOWNSAMPLE}/codon_specific_results"
    local codon_changes_file="${WORKDIR}/${popcode}_imputed_vcf_n${DOWNSAMPLE}/codon_changes.txt"
    ...
    if [[ "$PARALLEL_JOBS" -gt 1 ]]; then
        # Parallel processing using background processes and job control
        ...
        for i in "${!codon_changes[@]}"; do
            ...
            while [[ $active_jobs -ge $PARALLEL_JOBS ]]; do
                ...
                [[ $active_jobs -ge $PARALLEL_JOBS ]] && sleep 0.1
            done
        ...
        log "Waiting for remaining $active_jobs jobs to complete..."
        for pid in "${job_pids[@]}"; do
            wait "$pid"
            local exit_code=$?
            if [[ $exit_code -eq 0 ]]; then
